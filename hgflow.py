...
def _get_branchmap_items( repo ) :
    return repo.branchmap().iteritems() if ("5.0" <= util.version()) else repo.branchmap().items()
...
class Compatible( object ) :
    ...
    get_branchmap_items = staticmethod( _get_branchmap_items )
...
class Stream( object ) :
    ...
    def branches( self, openclosed = "open" ) :
        ...
        all_branches = []
        if (openclosed == "open") :
            for branch_fullname, heads in Compatible.get_branchmap_items( self.repo ) :
...
class Flow( object ) :
    ...
    def _strip( self, *arg, **kwarg ) :
        """
        Calls `commands.strip` with backward compatibility.
        """
        if ("5.0" <= util.version()) :
            kwarg["soft"] = None
        commands.strip( self.ui, self.repo, *arg, **kwarg )
